...
class KalFuzzy:
    ...
    def run_dbdata(self, abstable):
        ...
        abstable = abstable[abstable['soh'] <= 1]
        abstable = abstable.reset_index(drop=True)
        abstable = abstable[abstable['soh'] >= 0.8]
        abstable = abstable.reset_index(drop=True)
...
class DataCalculation:
    ...
    def Ampere_hour_integral(self, data_input, C_rate):
        ...
        for subgroup in grouped:
            ...
            # # print("表计算容量", cap)
            process_sub = {}
            process_sub['car_number'] = car_number
            process_sub['time'] = maxtime
            process_sub['cap'] = cap
            process_sub['soh'] = soh
            process_sub['min_soc'] = data_minsoc
            process_sub['max_soc'] = data_maxsoc
            process_sub['soc_gap'] = soc_gap
            process_sub['mileage'] = mileage
            process_sub['battery_min_temperature'] = battery_min_temperature
            process_sub['battery_max_temperature'] = battery_max_temperature
            process_sub['battery_avg_temperature'] = T_average
            process[chargeMode] = process_sub
        return process

    def Ampere_hour_integral1(self, data_input, C_rate):
        """[安时计分估计容量,针对每次充电数据]
        """
        subgroup = data_input
        # chargeMode = 0
        # for subgroup in grouped:
        battery_min_temperature = np.min(subgroup['battery_max_temperature'])
        battery_max_temperature = np.max(subgroup['battery_max_temperature'])
        mileage = max(subgroup['mileage'])
        T_average = np.mean(
            [np.mean(subgroup['battery_max_temperature']), np.mean(subgroup['battery_max_temperature'])])
        car_number = subgroup['car_number'].values[0]
        # # print("car_number",car_number)
    ...
    def get_singlecar_soh(self, car, socgap_threshold=20, minsoc_threshold_cur=40, maxsoc_threshold_cur=80, C_rate=159,
                          soh_rate=100):
        """
        [安时积分估计初始寿命]
        Arguments:
            ...
            if subgroup[0] > 0:
                ...
                if minsoc < minsoc_threshold_cur and socgap > socgap_threshold and \
                    ...
                    temptable = pd.DataFrame({'carNum': [car_number], 'Time': [mintime], 'time': [maxtime],
                                              'cap': [cap], 'Soh': [soh], 'temp_fix_soh': [temp_soh],
                                              'minSOC': [minsoc],
...
if __name__ == "__main__":
    ...
    data_input = pd.read_csv(os.path.join('D:/company/比亚迪项目/比亚迪需求每天调用一次每月调用一次/', 'df.csv'),
                             encoding='gbk')  # The data load
    # print("data_input",data_input)
